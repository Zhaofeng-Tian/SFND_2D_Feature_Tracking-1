...
int main(int argc, const char *argv[])
{
        ...
        for (const string & descriptorType : descriptorTypes)
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
            {
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    {
                        cout << detectorType << " detector is not compatible with " << descriptorType << endl;
                        continue;
                    }

                    totalProcessingTime = ((double)cv::getTickCount() - totalProcessingTime) /
                        cv::getTickFrequency();
    ...
    for (const auto & pair : matchStat)
    {
        ...
        averageProcessingTimeToAlg.emplace(1.0 * pair.second.totalProcessingTime / pair.second.matchedFramesCount,
            pair.first);
    }
